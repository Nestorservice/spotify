...
class MusicModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext),
    ...
    fun updateMetadataForTrack(index: Int, map: ReadableMap?, callback: Promise) =
        scope.launch {
            ...
            if (index < 0 || index >= musicService.tracks.size) {
                ...
                // CORRECTION : on extrait le Bundle de manière sûre
                val metadata = Arguments.toBundle(map) ?: return@launch callback.reject(
                    "invalid_metadata",
                    "Metadata cannot be null"
                )
                track.setMetadata(context, metadata, musicService.ratingType)
    ...
    fun getPlaybackState(callback: Promise) = scope.launch {
        if (verifyServiceBoundOrReject(callback)) return@launch
        // CORRECTION : on récupère le Bundle de manière sûre
        val stateBundle = musicService.getPlayerStateBundle(musicService.state) ?: Bundle()
